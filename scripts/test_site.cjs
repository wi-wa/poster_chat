...
async function main() {
  ...
  try {
    ...
    assert.equal(await hand.locator("#overallValue").textContent(), "95.0%");
    assert.equal(await hand.locator(".correlation-table tbody tr").count(), 2);
    ...
    assert.match(await hand.locator("#correlationCaption").textContent(), /1 checked/);
    const popupReady = page.waitForEvent("popup");
    await hand.locator("#distributionButton").click();
    const popup = await popupReady;
    await popup.locator(".dist-page").waitFor();
    assert.equal(await popup.locator(".dist-card").count(), 2);
    await popup.close();
